...
def extract_file(zip_file_path, file_info, tag, extract_to):
    try:
    ...
    try:
        with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
            ...
            with ThreadPoolExecutor() as executor:
                list(tqdm(executor.map(lambda file_info: extract_file(zip_file_path, file_info, tag, extract_to), file_infos), total=len(file_infos), desc="Extracting files", unit="file"))
